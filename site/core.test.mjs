...
test('initial scenario returns the expected decision contract',async()=>{const value=await compute(structuredClone(initialState));assert.equal(value.status,"Access denied");assert.equal(typeof value.summary,'string');assert.ok(value.metrics.length>=2);assert.ok(value.rows.length>=1)});
